...
public class MinesweeperServer {
    ...
    // This server only ever mutates two components, both of 
    // which are thread safe.
    // The first component, the board, is of type Board, a 
    // thread safe date type. This data type coordinates all
    // mutations to the board.  
    // The second is a reference to the number of connections
    // to the server, which is implemented using an AtomicInteger
    // object, a threadsafe data type
    ...
    private void handleConnection(Socket socket) throws IOException {
        ...
        String welcomeMsg = "Welcome to Minesweeper. Board: " + board.getColumns() + " columns by " + board.getRows() + " rows. Players: " + connections +
            ...
            for (String line = in.readLine(); line != null; line = in.readLine()) {
                ...
                if (output != null) {
                    if (output.equals("BOOM!") && !debug) {
                        ...
                        break;
                    } else if (output.equals("invalid input")) {
                        //do nothing
                    }
                    else {
                        out.println(output);
    ...
    private String handleRequest(String input) {
        ...
        if (tokens[0].equals("look")) {
            ...
            return "Valid commands are 'look', 'dig', 'flag', 'deflag', and 'bye'";
            ...
            if (tokens[0].equals("dig")) {
                // 'dig x y' request
                return board.processAction(x, y, Action.DIG);
            } else if (tokens[0].equals("flag")) {
                return board.processAction(x, y, Action.FLAG);
                
            } else if (tokens[0].equals("deflag")) {
                return board.processAction(x, y, Action.UNFLAG);
    ...
    public static void runMinesweeperServer(boolean debug, Optional<File> file, int sizeX, int sizeY, int port) throws IOException {
        ...
        Board currentBoard;
        if (file.isPresent()) {
            currentBoard = new Board(file.get());
        } 
        else {
            currentBoard = new Board(sizeX, sizeY);
        }

        MinesweeperServer server = new MinesweeperServer(port, debug, currentBoard);
        server.serve();
